...
draw_vpc_categorical <- function(project.name, item.number, simdata.list) {
  ...
  empirical <- read.csv(here("data","vpc_vali_data",
...
for(i in 1:14) {
  sim_yi <- read.csv(here("data","vpc_vali_data",paste0(project.name, "/ChartsData/VisualPredictiveCheck/y", 
...
vpc_basic2 <- vpc_basic2 %>%
  mutate(PROCTCAE = factor(PROCTCAE, levels = grouped_order))

p1 <- ggplot(vpc_basic2, aes(x = TIME)) +
  geom_ribbon(aes(ymin = exp_l, ymax = exp_h), fill = "lightskyblue", alpha = 0.7) +
...
#===============================================================================
sim_survival <- read.csv(here("data","vpc_vali_data",
                        paste0(project.name, "/ChartsData/VisualPredictiveCheck/survival_simulations.txt")),
                         header = TRUE) %>%
                        select(rep,ID,time)
...
vpc_vali <- ggplot(vpc_dropout2, aes(x = TIME)) +
  geom_ribbon(aes(ymin = exp_l, ymax = exp_h), fill = "lightskyblue", alpha = 0.7) +
  geom_line(aes(y = exp_obs), color = "royalblue4", linewidth = 1) +
  #geom_point(aes(y = exp_obs), color = "royalblue4",size = 1.2) +
  labs(x = "Time (days)", y = "Item Score") +
